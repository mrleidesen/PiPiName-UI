...
def get_source(source, validate, stroke_list):
    ...
    # 唐诗
        ...
        print('>>加载唐诗...')
        get_name_json('唐诗/poet.tang.' + str(random.randrange(0, 58000, 1000)), names, 'paragraphs', stroke_list)
    # 宋诗
    elif source == 6:
        print('>>加载宋诗...')
        get_name_json('宋诗/poet.song.' + str(random.randrange(0, 255000, 1000)), names, 'paragraphs', stroke_list)
    # 宋词
    elif source == 7:
        print('>>加载宋词...')
        get_name_json('宋词/ci.song.' + str(random.randrange(0, 22000, 1000)), names, 'paragraphs', stroke_list)
    else:
        print('词库号输入错误')
...
def get_name_dat(path, names, stroke_list):
    with open('data/' + path + '.dat', encoding='utf-8') as f:
        ...
        for i in range(0, size, random.randint(4, 20)):
            # 生成进度
...
def get_name_json(path, names, column, stroke_list):
    with open('data/' + path + '.json', encoding='utf-8') as f:
        ...
        progress = 0
        # start = random.randint(0, size - 100)
        for j in range(0, size, random.randint(4, 20)):
            # 生成进度
